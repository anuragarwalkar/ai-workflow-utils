...
import { defaultTemplates as fallbackDefaultTemplates } from '@ai-workflow-utils/data';
...
class TemplateDbService {
  adapter: JSONFile<TemplateDbData>;
  db: Low<TemplateDbData>;
  ...
  constructor() {
    ...
    this.adapter = new JSONFile<TemplateDbData>(dbPath);
    this.db = new Low<TemplateDbData>(this.adapter, { templates: [], settings: { lastUpdated: new Date().toISOString(), activeTemplates: {} } });
    logger.info(`Template database initialized at: ${dbPath}`);
  }

  async loadDefaultTemplates(): Promise<TemplateDbData> {
    try {
      const candidates = [
        path.join(process.cwd(), './libs/data/src/defaultTemplates.json'),
        path.join(process.cwd(), './dist/libs/data/defaultTemplates.json'),
        path.join(process.cwd(), './data/defaultTemplates.json'),
      ];

      let defaultTemplatesData: any = null;
      for (const candidate of candidates) {
        if (fs.existsSync(candidate)) {
          defaultTemplatesData = JSON.parse(fs.readFileSync(candidate, 'utf8'));
          break;
        }
      }

      if (!defaultTemplatesData) {
        defaultTemplatesData = fallbackDefaultTemplates;
      }

      const templatesWithTimestamps: TemplateItem[] = (defaultTemplatesData.templates || []).map((template: any) => ({
        ...template,
      ...
      return {
        ...
        settings: {
          version: defaultTemplatesData.settings?.version || '1.0.0',
          activeTemplates: defaultTemplatesData.settings?.activeTemplates || {},
  ...
  async init(): Promise<boolean> {
    try {
      ...
      if (!this.db.data || !this.db.data.templates) {
        ...
        await this.db.write();
        logger.info('Template database initialized with default templates');
      }
  ...
  async getTemplatesByType(issueType: string): Promise<TemplateItem[]> {
    try {
      await this.db.read();
  ...
  async getActiveTemplate(issueType: string): Promise<TemplateItem | undefined> {
    try {
      await this.db.read();
  ...
  async createTemplate(template: { name: string; issueType: string; content: string }): Promise<TemplateItem> {
    try {
      await this.db.read();
  ...
  async updateTemplate(id: string, updates: Partial<TemplateItem>): Promise<TemplateItem> {
    try {
      await this.db.read();
  ...
  async deleteTemplate(id: string): Promise<TemplateItem> {
    try {
      await this.db.read();
  ...
  async setActiveTemplate(issueType: string, templateId: string): Promise<TemplateItem> {
    try {
      await this.db.read();
      ...
      if (!this.db.data.settings) {
        this.db.data.settings = { lastUpdated: new Date().toISOString(), activeTemplates: {} };
      }
  ...
  async updateSettings(updates: Partial<TemplateSettings>): Promise<TemplateSettings> {
    try {
      await this.db.read();
  ...
  async resetToDefaults(): Promise<TemplateDbData> {
    try {
      await this.db.read();
  ...
  async exportTemplates(): Promise<{ templates: TemplateItem[]; exportedAt: string; version?: string }> {
    try {
      await this.db.read();
      return {
  ...
  async importTemplates(importData: { templates: any[] }): Promise<TemplateItem[]> {
    try {
      await this.db.read();
      ...
      const importedTemplates: TemplateItem[] = importData.templates.map(template => ({
        ...template,
