import appStateDbService from '../appStateDbService.ts';
...
const DEFAULT_TILES: TileLayoutItem[] = [
...
class TileConfigDbService {
  async init(): Promise<void> {
    let tiles: TileLayoutItem[] | null = await appStateDbService.getState<TileLayoutItem[]>(CONFIG_KEY);
    ...
    if (!tiles || tiles.length === 0) {
      ...
      DEFAULT_TILES.forEach(defaultTile => {
        const existing = (tiles as TileLayoutItem[]).find(t => t.id === defaultTile.id);
        if (!existing) {
          (tiles as TileLayoutItem[]).push(defaultTile);
          updated = true;
        } else {
  ...
  async getConfig(): Promise<TileLayoutItem[]> {
    await this.init();
    return (await appStateDbService.getState<TileLayoutItem[]>(CONFIG_KEY)) || DEFAULT_TILES;
  }

  async updateConfig(tilesArray: TileLayoutItem[]): Promise<TileLayoutItem[]> {
    await this.init();
