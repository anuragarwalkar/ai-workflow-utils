...
class ChatService {
   ...
   */
  static async generateResponse({ message, conversationHistory = [], options = {} }) {
    try {
      ...
      if (conversationHistory.length > 0) {
        await this.addConversationHistoryToMemory(sessionId, conversationHistory);
      }
   ...
   */
  static async generateStreamingResponse({ message, conversationHistory = [], options = {} }, res) {
    try {
  ...
  static async addConversationHistoryToMemory(sessionId, conversationHistory) {
    try {
      ...
      for (const msg of conversationHistory) {
        if (msg.role === 'user') {
          await ChatLangChainService.conversationMemories.get(sessionId)?.chatMemory?.addUserMessage(msg.content);
        } else if (msg.role === 'assistant') {
          await ChatLangChainService.conversationMemories.get(sessionId)?.chatMemory?.addAIChatMessage(msg.content);
        }
      }
      
      logger.info(`Added ${conversationHistory.length} messages to memory for session ${sessionId}`);
   ...
   */
  static async testChatFunctionality(testMessage = 'Hello, can you help me?', providerName = null, options = {}) {
    try {
      return await ChatLangChainService.testChatFunctionality(testMessage, providerName, options);
    } catch (error) {
