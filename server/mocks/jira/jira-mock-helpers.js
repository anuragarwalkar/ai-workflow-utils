...
const getIssueMetadata = issueData => {
  const issueType =
    mockData.issueTypes.find(type => type.id === issueData.issuetype?.id) || mockData.issueTypes[0];

  const priority =
    mockData.priorities.find(p => p.id === issueData.priority?.id) || mockData.priorities[2];
...
export const searchIssues = (jql, startAt = 0, maxResults = 50) => {
  ...
  if (jql.includes('project =')) {
    ...
    if (projectMatch) {
      ...
      filteredIssues = filteredIssues.filter(issue => issue.fields.project.key === projectKey);
    }
  }
  ...
  if (jql.includes('status =')) {
    ...
    if (statusMatch) {
      ...
      filteredIssues = filteredIssues.filter(issue => issue.fields.status.name === statusName);
    }
  }
