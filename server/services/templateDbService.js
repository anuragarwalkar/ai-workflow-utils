...
class TemplateDbService {
  ...
  async loadDefaultTemplates() {
    try {
      const defaultTemplatesPath = path.join(__dirname, '../data/defaultTemplates.json')
  ...
  async updateTemplate(id, updates) {
    try {
      ...
      this.db.data.templates[index] = {
        ...this.db.data.templates[index],
        ...updates,
        variables: updates.content ? this.extractVariables(updates.content) : this.db.data.templates[index].variables,
        updatedAt: new Date().toISOString()
      }
  ...
  async deleteTemplate(id) {
    try {
      ...
      if (this.db.data.settings.activeTemplates[issueType] === id) {
        const alternativeTemplate = this.db.data.templates.find(t => 
          t.issueType === issueType && t.id !== id
        )
